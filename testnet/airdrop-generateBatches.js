...
async function main() {
    ...
    const inactivesData = await readCsvFile('testnet-airdrop-inactiveusers.csv');
    console.log(`Read ${inactivesData.length} inactive addresses`)
    let activesData = await readCsvFile('testnet-airdrop-activeusers.csv');
    ...
    const inactiveAmount = '1000000000'; // 1 JAM
    var transfersData = [];
    for (const inactive of inactivesData) {
        transfersData.push({
            address_ss58: inactive.address_ss58,
            airdrop_amount: inactiveAmount
        })
    }
    ...
    const MAX_MYTH_PER_GROUP = 75200;
    ...
    const decimalString = '0000000000'; // 10 zeroes for JAM
